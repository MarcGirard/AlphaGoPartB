...
#       
        ...
        for move in moves: # this is the max turn(1st level of minimax), so next should be min's turn
            ...
            print("init",move)
            moveVal = self.alphaBeta_pruning(newBoard, self.colour, self.depth, 'min', self.opponentColour, alpha, beta, self.board)
            ...
            if best == None or moveVal[0] > best:
                ...
                if move[0] == "MOVE":
                    ...
                    equalBooms.append((moveVal[1],move))
        ...
        if len(equalBooms) != 0 :
            equalBooms.sort(key = lambda x: x[0], reverse=True)
            if equalBooms[0][0] == 0:
                bestMove = equalBooms[0][1]
        
        elif  len(equalMoves) != 0:
    ...
    def alphaBeta_pruning(self, board, colour, depth, turn, opponentColour, alpha, beta, previousState):
        if depth > 1: #Comes here depth-1 times and goes to else for leaf nodes.
            ...
            if turn == 'max':
                ...
                for move in moves:
                    ...
                    if opti[0] == None or beta > opti[0]:
                        value = self.alphaBeta_pruning(nextBoard, colour, depth, 'min', opponentColour, alpha, beta, board)
                        
                        if value[0] == None:
                            value[0] = -float("inf")

                        if opti[0] == None or value[0] > opti[0]:
                            opti[0] = value[0]
                ...
                for move in moves:
                    ...
                    if alpha == None or opti[0] == None or alpha < opti[0]: #None conditions are to check for the first times
                        value = self.alphaBeta_pruning(nextBoard, colour, depth, 'max', opponentColour, alpha, beta, board)
                        
                        if value[0] == None:
                            value[0] = float("inf")

                        if opti[0] == None or value[0] < opti[0]: #opti[0] = None for the first time
                            opti[0] = value[0]
                        if opti[0] < beta:
                            beta = opti[0]
            ...
            # value = fvalue - ivalue
            value[0] = sum(board.values())
            print("alpa before the frac", value)
            if value[0] == 0:
                # Change in number of our tokens
                deltaSelf = abs(sum([board[position] for position in board if board[position] > 0 ])) - abs(sum([previousState[position] for position in previousState if previousState[position] > 0 ]))

                # Change in number of opponent tokens
                deltaOpponent = abs(sum([board[position] for position in board if board[position] < 0 ])) - abs(sum([previousState[position] for position in previousState if previousState[position] < 0 ]))

                if deltaOpponent != 0 and deltaSelf != 0:               
                    value[0] += (1/100)*(abs(deltaOpponent)/abs(deltaSelf))
